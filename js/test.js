...
/* ===== 1.4 Hard fallback for lazy elements ===== */
(function hydrateLazyIfNeeded() {
try {
  document
    .querySelectorAll('img[data-lazy-src], iframe[data-lazy-src]')
    .forEach(el => {
      // If for some reason IntersectionObserver didn't fire,
      // or JS ran late, make sure these still get a src.
      const hasSrc = el.getAttribute('src');
      const lazy   = el.getAttribute('data-lazy-src');
      if (!hasSrc && lazy) {
        el.setAttribute('src', lazy);
        // optional: el.removeAttribute('data-lazy-src');
      }
    });
} catch (err) {
  console.error('[Lazy] hydrateLazyIfNeeded crashed:', err);
}
})();
...
/* ===== 2. SoundCloud image-controlled player (Option B: single hidden iframe) ===== */
  (function initSCController() {
    try {
      ...
      document.querySelectorAll('.slot--sc .sc-trigger').forEach(trigger => {
        ...
        trigger.addEventListener('click', async (e) => {
          ...
          try {
            ...
            console.error('[SC] click handler error:', err);
          } finally {
      ...
      console.error('[SC] initSCController crashed:', err);
    }
  })();
 ...
 /* ===== 5. Overlay click-to-open (delegated) ===== */
  try {
    ...
    if (overlay && ovCard && ovImg) {
      console.log('[Overlay] init: found overlay elements');
    ...
    console.error('[Overlay] init crashed:', err);
  }
});
